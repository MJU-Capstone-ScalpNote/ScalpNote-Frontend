...
const CommunityScreen = () => {
  ...
  const fetchPostDetails = async (postId) => {
    try {
      const response = await axios.get(`http://43.201.108.238:8080/boards/${postId}/simple-boards`);
  ...
  const postComment = async () => {
    ...
    try {
      const response = await axios.post(`http://43.201.108.238:8080/boards/comment/${selectedPost.postId}`, {
        ...
        <View style={styles.postDetailContainer}>
          ...
          {selectedPost.imageUrl && (
            <Image
              ...
              onError={(e) => console.error("Image loading error: ", e.nativeEvent.error)} // 이미지 로딩 에러 로그 출력
            />
          )}
          ...
          <View style={styles.commentInputContainer}>
            {replyTo && (
              <Text style={styles.replyingToText}>
                답글 대상: {comments.find((comment) => comment.id === replyTo)?.writer?.name}
              </Text>
            )}
            <TextInput
